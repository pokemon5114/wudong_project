  ...
  <view v-if="goods" class="detail">
    ...
    <view class="card">
      <view class="price-row">
        ...
        <text v-if="goods.marketPrice" class="market">¥{{ goods.marketPrice }}</text>
        <text class="sales">已售 {{ goods.sales || 0 }}</text>
      </view>
      <text class="title">{{ goods.title }}</text>
      <text v-if="goods.subtitle" class="subtitle">{{ goods.subtitle }}</text>
    </view>
    ...
    <view class="card">
      <text class="card__title">用户评价（{{ goods.evaluationCount || 0 }}）</text>
      <view v-for="e in evaluations" :key="e.id" class="eval">
        <text class="eval__score">{{ '★'.repeat(e.score) }}</text>
        <text class="eval__content">{{ e.content }}</text>
      </view>
      <wd-empty v-if="!evaluations.length" text="暂无评价" />
    </view>
...
function buyNow() {
  if (!requireLogin() || !goods.value) return;
  setDraft({ orderType: 'goods', title: goods.value.title, items: [buildItem()] });
...
onLoad(async (options) => {
  ...
  const res = await goodsApi.evaluations(id, { page: 1, size: 3 });
